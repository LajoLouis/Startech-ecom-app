...
function Header() {
  const { open, setOpen, cartItems, cartCount, showAndHide } =
  ...
  const showHeader = (
    <div
      className={` fixed top-0 z-[20] w-full flex items-center justify-between py-[15px] px-[30px] bg-blue-950`}
    >
      <div>
        <a href="">
      ...
      <nav
        className={`xs:hidden space-x-4 text-white text-[15px] lg:flex sm:visible ${
          open ? "hidden" : ""
        }`}
      >
        <Link to="/">Home</Link>
      ...
      ></div>
      <div
        className={`fixed lg:hidden left-0 top-0 xs:w-[150px] sm:w-[200px] rounded-br-[10px] xs:h-[50%] sm:h-[70%] overflow-auto z-[30] bg-blue-950  transition-all duration-1000 ${
          open
            ? "translate-x-[0px] bg-opacity-90"
            : "translate-x-[-500px] bg-opacity-100"
        }`}
      >
    ...
    <div className="fixed top-0 z-[20] w-full flex items-center justify-between py-[15px] px-[30px] bg-blue-950">
      ...
      >
        ...
        <Link to="/cart" className="relative">
          ...
          <div className="absolute bottom-2 left-3 text-blue-950 bg-white text-center rounded-full h-4 w-4 text-[10px] font-extrabold">
            {cartCount}
          </div>
        </Link>
